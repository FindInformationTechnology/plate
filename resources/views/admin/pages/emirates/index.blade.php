                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
                            ...
                            <tbody class="text-gray-600 fw-semibold">
                                ...
                                <tr>
                                    <td>{{ $emirate->id }}</td>
                                    <td>{{ $emirate->name }}</td>
                                    <td>{{ $emirate->created_at->format('d M Y, h:i a') }}</td>
                                    <td class="text-end">
                                        <a href="#" class="btn btn-light btn-active-light-primary btn-sm btn-data" data-id="{{ $emirate->id }}" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Actions
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                            <div class="menu-item px-3">
                                                <a href="#" class="menu-link px-3 btn-edit" data-id="{{ $emirate->id }}">Edit</a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="#" class="menu-link px-3" data-kt-users-table-filter="delete_row" data-id="{{ $emirate->id }}">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
@include('admin.pages.emirates.partials.emirate_crud')
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-edit').click(function() {
            ...
            $.get('{{ route("admin.emirates.edit", ":id") }}'.replace(':id', id), function(data) {
                // console.log(data.emirate.name.en);
                // Set form action URL with the correct ID
                $('#editForm').attr('action', '{{ route("admin.emirates.update", "") }}/' + id);
                ...
                if (data.emirate.icon) {
                    var iconUrl = '{{ asset("storage") }}/' + data.emirate.icon;
                    $('#edit_emirate_icon_wrapper').css('background-image', 'url(' + iconUrl + ')');
                    $('#edit_emirate_icon_preview').css('background-image', 'url(' + iconUrl + ')');
                } else {
                    $('#edit_emirate_icon_wrapper').css('background-image', 'url({{ asset("assets/media/svg/avatars/blank.svg") }})');
        ...
        $('[data-kt-users-table-filter="delete_row"]').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{ route("admin.emirates.destroy", "") }}/' + emirateId,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
