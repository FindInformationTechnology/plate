...
@section('title', __('message.Contact_Us') . ' - ' . config('app.name'))
@section('meta_description', __('message.Contact_Meta_Description'))
                ...
                <form method="POST" action="{{ route('contact.send') }}" class="space-y-5">
                    ...
                    <div class="mb-3">
                        <input type="text" name="name" placeholder="{{ __('message.Your_Name') }}"
                            class="form-control rounded-md @error('name') is-invalid @enderror" value="{{ old('name') }}"
                            required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        <input type="email" name="email" placeholder="{{ __('message.Your_Email') }}"
                            class="form-control rounded-md @error('email') is-invalid @enderror" value="{{ old('email') }}"
                            required>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        <textarea name="message" rows="5" placeholder="{{ __('message.Your_Message') }}"
                            class="form-control rounded-md resize-none @error('message') is-invalid @enderror"
